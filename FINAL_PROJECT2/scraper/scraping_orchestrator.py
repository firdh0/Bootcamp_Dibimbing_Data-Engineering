...
class ScrapingOrchestrator:
    ...
    def __init__(self, restaurant_list: list, datastore_manager: DatastoreManager, max_workers: int = 5) -> None:
        """
    ...
    def run(self) -> tuple[pd.DataFrame, pd.DataFrame, pd.DataFrame, pd.DataFrame, dict]:
        ...
        with concurrent.futures.ThreadPoolExecutor(max_workers=self.max_workers) as executor:
            futures = {
                executor.submit(self.scraper.get_single_restaurant_details, restaurant, index + 1, len(self.restaurant_list), self.datastore_manager): restaurant
